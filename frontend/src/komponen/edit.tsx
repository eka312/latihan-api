...
function Edit({ idBuku, dataBuku,  setShowEdit, getBuku}: EditProps) {
    const [formData, setFormData] = useState({
        judul: dataBuku.judul ?? "",
        pengarang: dataBuku.pengarang ?? "",
        penerbit: dataBuku.penerbit ?? "",
        tahun_terbit: dataBuku.tahun_terbit?.toString() ?? "",
        kategori: dataBuku.kategori ?? "",
        jumlah: dataBuku.jumlah?.toString() ?? "",
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await axios.put(`http://127.0.0.1:8000/api/buku/${idBuku}`, payload);
        ...
        setShowEdit(false);
            ...
            console.error("Gagal update buku:", error.message);
